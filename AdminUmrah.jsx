...
export default function AdminUmrah() {
  ...
  const load = () => apiGet("/umrah").then(setItems);
  useEffect(() => { load(); }, []);

  const submit = async (e) => {
    e.preventDefault();
    await apiPost("/umrah", form, token);
    setForm({ title: "", description: "" });
    load();
  };

  const remove = async (id) => {
    await apiDelete(`/umrah/${id}`, token);
    load();
  };

  return (
    <AdminLayout title="Manage Umrah">
      <form onSubmit={submit} className="rounded-2xl bg-white p-6 shadow ring-1 ring-slate-200">
        <div className="grid gap-4">
          ...
          <button className="rounded-xl bg-emerald-600 py-3 text-white">Add Umrah Step</button>
        </div>
      </form>
